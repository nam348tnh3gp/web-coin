...
CPU Mining Edition - With interactive setup
"""
...
import getpass  # Thêm thư viện để nhập mật khẩu ẩn
...
def print_config_menu():
    """In menu cấu hình"""
    print(f"{Fore.YELLOW}{Style.BRIGHT}📋 CONFIGURATION MENU{Style.RESET_ALL}")
    print("═" * 50)
    print(" 1. Set Wallet Address")
    print(" 2. Set Password")
    print(" 3. Set Difficulty Level")
    print(" 4. Set Number of Threads")
    print(" 5. Set Rig Identifier")
    print(" 6. Toggle Debug Mode")
    print(" 7. Save and Start Mining")
    print(" 8. Exit")
    print("═" * 50)
...
def input_wallet_address():
    ...
    while True:
        ...
        print("Example: W_0488269778421338855a2442815747a0b151c3389f60f27bf70b868578c5ab...")
        ...
        # Thử kiểm tra với server (tùy chọn)
        try:
            print(f"{Fore.CYAN}⏳ Checking wallet with server...{Style.RESET_ALL}")
            r = requests.get(f"{SERVER_URL}/balance/{addr}", timeout=5)
            if r.status_code == 200:
                balance = r.json().get('balance', 0)
                print(f"{Fore.GREEN}✅ Wallet verified! Balance: {balance} WebCoin{Style.RESET_ALL}")
            else:
                print(f"{Fore.YELLOW}⚠️ Wallet may not exist, but you can still continue{Style.RESET_ALL}")
        except:
            print(f"{Fore.YELLOW}⚠️ Could not verify wallet (server offline?), continuing anyway{Style.RESET_ALL}")
...
def input_password():
    ...
    print("Enter the password you used when creating this wallet")
    
    while True:
        pwd = getpass.getpass(f"{Fore.YELLOW}Password: {Style.BRIGHT}")
        if not pwd:
        ...
        pwd2 = getpass.getpass(f"{Fore.YELLOW}Confirm password: {Style.BRIGHT}")
        if pwd != pwd2:
...
def input_difficulty():
    ...
    print(" 1 - Low    (Faster mining, lower reward)")
    print(" 2 - Medium (Balanced speed and reward)")
    print(" 3 - High   (Slower mining, higher reward)")
...
def input_threads():
    ...
    print("Recommended: Use all cores for maximum hashrate")
    print("You can use fewer cores to keep your computer responsive")
...
def input_identifier():
    ...
    print("Leave empty for no identifier")
...
def toggle_debug(current):
    ...
    print("Debug mode shows detailed information for troubleshooting")
...
def interactive_setup():
    ...
    print("You'll need your wallet address and password.")
    ...
    print(f"  Password:   {'*' * len(config_data['password'])}")
...
class Algorithms:
    ...
    def WEBCOIN_S1(last_h, target_prefix, diff, intensity):
        ...
        max_nonce = 100000 * diff  # Giới hạn nonce
    ...
    while running:
        try:
            ...
            if start_diff == "LOW":
                ...
                try:
                    info = requests.get(f"{SERVER_URL}/info", timeout=5).json()
...
if __name__ == "__main__":
    ...
    # In banner bắt đầu
    print_banner()
    print(f"{Fore.GREEN}{Style.BRIGHT}🚀 STARTING MINER{Style.RESET_ALL}")
    print("═" * 50)
    ...
    try:
        while running:
            time.sleep(5)  # Cập nhật stats mỗi 5 giây
